...
Flotr.Graph = Class.create({
        ...
        for (i = neededColors - 1; i > -1; --i) {
            ...
            if (c) {
                ...
                if (Object.isNumber(c))
                    ...
                    usedColors.push(Flotr.Color.parse(c));
            }
        }
    ...
    processColor: function(color, options) {
        ...
        if (color instanceof Flotr.Color)
            return color.adjust(null, null, null, options.opacity).toString();
        if (Object.isString(color))
            return Flotr.Color.parse(color).scale(null, null, null, options.opacity).toString();

        var grad = color.colors ? color : {colors: color};

        if (!options.ctx) {
            if (!Object.isArray(grad.colors))
                return 'rgba(0, 0, 0, 0)';
        ...
        if (/top/i.test(grad.start))
            options.x1 = 0;
        if (/left/i.test(grad.start))
            options.y1 = 0;
        if (/bottom/i.test(grad.end))
            options.x2 = 0;
        if (/right/i.test(grad.end))
            options.y2 = 0;
    ...
    loadDataGrid: function() {
        ...
        return this.seriesData = dg.sortBy(function(v) {
            return v[0]
        });
    },
    /**
            ...
            for (i = 0; i < s.length; ++i) {
                ...
                if (data.length > 0 && !s[i].hide) {
                    for (h = data.length - 1; h > -1; --h) {
                        ...
                        if ((x <= 0) && (xaxis.options.scaling === 'logarithmic'))
                            continue;
                        ...
                        for (j = 1; j < data[h].length; j++) {
                            ...
                            // Logarithm is only defined for values > 0
                            if ((y <= 0) && (yaxis.options.scaling === 'logarithmic'))
                                continue;
    ...
    calculateRange: function(axis, type) {
        ...
        if (o.scaling === 'logarithmic') {
            if (min < 0)
                min = max / o.base;  // Could be the result of widening

            var maxexp = Math.log(max);
            if (o.base != Math.E)
                maxexp /= Math.log(o.base);
            maxexp = Math.ceil(maxexp);

            var minexp = Math.log(min);
            if (o.base != Math.E)
                minexp /= Math.log(o.base);
        ...
        if (o.max == null && o.autoscale) {
            ...
            if (axis.max > 0 && axis.datamax <= 0 && axis.datamax != axis.datamin)
                axis.max = 0;
            ...
            if (axis.options.showLabels) {
                for (i = 0; i < axis.ticks.length; ++i) {
                    ...
                    if (!tick.label || tick.label.length == 0 ||
                            ...
                            (this.plotOffset.left + axis.d2p(tick.v) > this.canvasWidth))
                        continue;
            ...
            if (axis.options.showLabels) {
                for (i = 0; i < axis.ticks.length; ++i) {
                    ...
                    if (!tick.label || tick.label.length == 0 ||
                            ...
                            (this.plotOffset.top + axis.d2p(tick.v) > this.canvasHeight))
                        continue;
            ...
            if (!options.HtmlText && this.textEnabled && !$(legend.container)) {
                ...
                if (fragments.length > 0) {
                    ...
                    if (options.legend.container != null) {
                        ...
                        if (p.charAt(0) == 'n')
                            pos += 'top:' + (m + plotOffset.top) + 'px;bottom:auto;';
                        else if (p.charAt(0) == 's')
                            pos += 'bottom:' + (m + plotOffset.bottom) + 'px;top:auto;';
                        if (p.charAt(1) == 'e')
                            pos += 'right:' + (m + plotOffset.right) + 'px;left:auto;';
                        else if (p.charAt(1) == 'w')
                            pos += 'left:' + (m + plotOffset.left) + 'px;right:auto;';
            ...
            else
            {
                ...
                portions = this.series.collect(function(hash, index) {
                    if (hash.pie.show && hash.data[0][1] !== null)
                        ...
                        sum = portions.pluck('value').pluck(1).inject(0, function(acc, n) {
                    return acc + n;
                }),
                        fraction = 0.0,
        ...
        if (n.series && (n.mouse && n.mouse.track && !prevHit || (prevHit /*&& (n.x != prevHit.x || n.y != prevHit.y)*/))) {
            ...
            if (!n.mouse.relative) { // absolute to the canvas
                ...
                if (!s.bars.show && !s.pie.show) {
                    if (p.charAt(0) == 'n')
                        pos += 'bottom:' + (m - plotOffset.top - n.yaxis.d2p(n.y) + this.canvasHeight) + 'px;top:auto;';
                    else if (p.charAt(0) == 's')
                        pos += 'top:' + (m + plotOffset.top + n.yaxis.d2p(n.y)) + 'px;bottom:auto;';
                    if (p.charAt(1) == 'e')
                        pos += 'left:' + (m + plotOffset.left + n.xaxis.d2p(n.x)) + 'px;right:auto;';
                    else if (p.charAt(1) == 'w')
                        pos += 'right:' + (m - plotOffset.left - n.xaxis.d2p(n.x) + this.canvasWidth) + 'px;left:auto;';
                }
    ...
    drawTooltip: function(content, x, y, options) {
        ...
        if (x !== null && y !== null) {
            if (!options.relative) { // absolute to the canvas
                if (p.charAt(0) == 'n')
                    style += 'top:' + (m + plotOffset.top) + 'px;bottom:auto;';
                else if (p.charAt(0) == 's')
                    style += 'bottom:' + (m + plotOffset.bottom) + 'px;top:auto;';
                if (p.charAt(1) == 'e')
                    style += 'right:' + (m + plotOffset.right) + 'px;left:auto;';
                else if (p.charAt(1) == 'w')
                    style += 'left:' + (m + plotOffset.left) + 'px;right:auto;';
            }
...
Flotr.Date = {
    ...
    getFormat: function(time, span) {
        ...
        if (time < tu.second)
            return "%h:%M:%S.%s";
        else if (time < tu.minute)
            return "%h:%M:%S";
        else if (time < tu.day)
            return (span < 2 * tu.day) ? "%h:%M" : "%b %d %h:%M";
        else if (time < tu.month)
            return "%b %d";
        else if (time < tu.year)
            return (span < tu.year) ? "%b" : "%b %y";
        else
            return "%y";
    },
    ...
    generator: function(axis) {
        ...
        switch (axis.tickUnit) {
            case "millisecond":
                d.setUTCMilliseconds(Flotr.floorInBase(d.getUTCMilliseconds(), axis.tickSize));
                break;
            case "second":
                d.setUTCSeconds(Flotr.floorInBase(d.getUTCSeconds(), axis.tickSize));
                break;
            case "minute":
                d.setUTCMinutes(Flotr.floorInBase(d.getUTCMinutes(), axis.tickSize));
                break;
            case "hour":
                d.setUTCHours(Flotr.floorInBase(d.getUTCHours(), axis.tickSize));
                break;
            case "month":
                d.setUTCMonth(Flotr.floorInBase(d.getUTCMonth(), axis.tickSize));
                break;
            case "year":
                d.setUTCFullYear(Flotr.floorInBase(d.getUTCFullYear(), axis.tickSize));
                break;
        }
...
Flotr.addType('lines', {
    ...
    plot: function(series, offset, incStack) {
        ...
        for (i = 0; i < length; ++i) {
            ...
            if (xa.options.scaling === 'logarithmic' && (data[i][0] <= 0 || data[i + 1][0] <= 0))
                continue;
            if (ya.options.scaling == 'logarithmic' && (data[i][1] <= 0 || data[i + 1][1] <= 0))
                continue;
...
Flotr.addType('markers', {
    ...
    plot: function(x, y, label, options) {
        ...
        if (options.position.indexOf('c') != -1)
            left -= dim.width / 2 + margin;
        else if (options.position.indexOf('l') != -1)
            left -= dim.width;

        if (options.position.indexOf('m') != -1)
            top -= dim.height / 2 + margin;
        else if (options.position.indexOf('t') != -1)
            top -= dim.height;
...
Flotr.addPlugin('spreadsheet', {
    options: {
        ...
        tabGraphLabel: 'Gráfico',
        tabDataLabel: 'Dados',
        toolbarDownload: 'Download Planilha', // @todo: add better language support
    ...
    callbacks: {
        'flotr:afterconstruct': function() {
            ...
            ss.tabs.graph.observe('click', function() {
                ss.showTab('graph')
            });
            ss.tabs.data.observe('click', function() {
                ss.showTab('data')
            });
        }
    },
